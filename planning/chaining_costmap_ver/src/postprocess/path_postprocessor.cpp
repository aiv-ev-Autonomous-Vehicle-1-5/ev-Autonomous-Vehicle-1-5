...
//
//   ① prune 먼저: 점 수를 줄여야 후속 단계의 비용이 감소하고,
//      불필요한 점에 의한 노이즈 증폭을 방지
//
//   ② resample이 smooth 앞에 오는 이유:
//      prune 후 점 간격이 불균등하다 (대각선 vs 직교, shortcut 거리 차이).
//      이동 평균 필터는 등간격 점에서 동작해야 편향 없는 평활화가 가능하다.
//      불균등 간격에서 smooth를 적용하면 밀집 구간에 가중치가 편향된다.
//
//   ③ smooth: 등간격 리샘플링된 점에 이동 평균을 적용하여
//      잔여 꺾임을 균등하게 완화
//
//   ④ curvature_clamp: smooth 후 남은 급커브를 차량 최소 회전 반경 이내로 제한
//
