# chaining_costmap_ver 패키지 구현 계획
...
**1-4. debug_publish.hpp** — 그대로 복사, namespace만 `chaining_costmap_ver`로 변경
...
**4-2. package.xml** — mr_ver 기반, 이름만 `chaining_costmap_ver`
...
1. **빌드**: `cd ~/ev_ws/planning && colcon build --packages-select chaining_costmap_ver`
2. **실행**: `ros2 launch chaining_costmap_ver planning_lc.launch.py`
