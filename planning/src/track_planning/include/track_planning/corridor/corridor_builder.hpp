 ...
 * 알고리즘:
 *   1. 참조점(c_end) + 접선(t_end) 결정 (항상 ego_pos + ego_heading)
...
public:
  ...
  struct Input
  {
    ...
    Point2D ego_pos{0.0, 0.0};            // ego 위치 (base_link 원점, 항상 (0,0))
    Point2D ego_heading{1.0, 0.0};        // ego heading 단위벡터 (항상 +x 전방)
  };
...
private:
  ...
  /// 항상 ego_pos + ego_heading 반환 (이전 프레임 의존 제거)
  /// @return {c_end: 참조점, t_end: 참조 접선 단위벡터}
  std::pair<Point2D, Point2D> determine_reference(const Input & in);
  ...
  /// seed 점 찾기: 참조 접선 방향 전방 forward_range 이내에서 ego에 가장 가까운 점
  /// 콘 우선: 콘에서 seed가 발견되면 차선 seed보다 우선 사용
  /// @param seed_out     찾은 seed 점 (출력)
  /// @param seed_source  0 = cone, 1 = lane (출력)
  /// @param seed_index   해당 배열에서의 인덱스 (출력)
  /// @return             seed 발견 여부
  bool find_seed(
    const std::vector<Point2D> & lane_pts,
    const std::vector<Point2D> & cone_pts,
