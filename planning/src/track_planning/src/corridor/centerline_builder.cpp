 ...
 * ├─────────────────────────────────────────────────────────────────────┤
 * │                                                                     │
 * │  좌측 경계:  L0──L1──L2──L3──L4──L5                                │
 * │               \  / \  / \  / \  / \                                │
 * │                \/   \/   \/   \/   \                               │
 * │           CDT  /\   /\   /\   /\    \  ← Delaunay 삼각분할         │
 * │               /  \ /  \ /  \ /  \    \                             │
 * │  우측 경계:  R0──R1──R2──R3──R4──R5                                │
 * │                                                                     │
 * │  삼각형 필터링 후:                                                  │
 * │    - 좌+우 꼭짓점을 모두 포함하는 삼각형만 선택 (Filter A)         │
 * │    - 트랙을 "가로지르는" 얇은 삼각형 선호 (Filter B)               │
 * │                                                                     │
 * │  외심(circumcenter) 추출:                                           │
 * │    L0──L1──L2──L3──L4──L5                                          │
 * │         ·    ·    ·    ·     ← 외심 = Voronoi 정점                 │
 * │    R0──R1──R2──R3──R4──R5        = 양 경계에서 등거리               │
 * │                                                                     │
 * │  인접 그래프 체이닝:                                                │
 * │         ·────·────·────·     ← 외심을 순서대로 연결 = 센터라인     │
 * │                                                                     │
 ...
 *     4. 외심(circumcenter) 추출 — Voronoi 이중 그래프의 정점
 *     5. 삼각형 인접 그래프 기반 체이닝 — Voronoi 엣지 = medial axis
 *     6. 이동평균 스무딩 + 리샘플링
 *
 ...
 *
 * DTR과 Voronoi 관계:
 *   - Delaunay 삼각분할의 "이중 그래프(dual graph)"가 Voronoi 다이어그램
 *   - 삼각형의 외심 = Voronoi 정점 (인접 정점들에서 등거리)
 *   - 인접 삼각형의 외심을 연결 = Voronoi 엣지 = medial axis
 *   - 좌/우 경계 사이의 medial axis = 중앙선(centerline)
 *
