 ...
 *   ┌─────────────────────────────────────────────────────────┐
 *   │ 0단계: 전처리 (Preprocess)                              │
 *   │   - 신뢰도(confidence)가 min_confidence 미만인 점 제거   │
 *   │   - 노이즈/오탐 사전 제거                                │
 *   ├─────────────────────────────────────────────────────────┤
 ...
 * ══════════════════════════════════════════════════════════════
 */
#ifndef PLANNING_LC_VER__CHAINER__DIRECTION_CHAINER_HPP_
#define PLANNING_LC_VER__CHAINER__DIRECTION_CHAINER_HPP_
 ...
 * @brief 7단계 파이프라인으로 경계점을 좌/우 체인으로 분류하는 핵심 클래스
 *
 * [사용 방법]
...
private:
  // ═══════════════════════════════════════════════════════════
  // 0단계: 전처리 (Preprocess) — 신뢰도 기반 노이즈 제거
  // ═══════════════════════════════════════════════════════════
  /**
   * @brief 신뢰도(confidence)가 min_confidence 미만인 점 제거
   *
   * [왜 필요한가?]
   *   BBox 검출기나 차선 인식기는 오탐/저신뢰 결과를 내놓을 수 있다.
   *   이런 점들이 그래프에 포함되면 잘못된 연결이 생기므로
   *   파이프라인 맨 처음에 제거한다.
   *
   * @param points  원본 경계점 배열
   * @param cp      chainer 파라미터 (min_confidence 사용)
   * @return 신뢰도 필터를 통과한 점들만 포함된 새 배열
   */
...
#endif  // PLANNING_LC_VER__CHAINER__DIRECTION_CHAINER_HPP_
