...
```
planning/src/planning_lc_ver/
├── CMakeLists.txt
├── package.xml
├── config/planning_lc.yaml
├── launch/planning_lc.launch.py
├── include/planning_lc_ver/
│   ├── common/
...
**1-4. debug_publish.hpp** — 그대로 복사, namespace만 `planning_lc_ver`로 변경
...
**4-2. package.xml** — mr_ver 기반, 이름만 `planning_lc_ver`
...
1. **빌드**: `cd ~/ev_ws/planning && colcon build --packages-select planning_lc_ver`
2. **실행**: `ros2 launch planning_lc_ver planning_lc.launch.py`
