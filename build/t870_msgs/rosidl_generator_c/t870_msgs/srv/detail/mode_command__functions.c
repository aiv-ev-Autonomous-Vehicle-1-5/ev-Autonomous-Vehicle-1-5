// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from t870_msgs:srv/ModeCommand.idl
// generated code does not contain a copyright notice
#include "t870_msgs/srv/detail/mode_command__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

bool
t870_msgs__srv__ModeCommand_Request__init(t870_msgs__srv__ModeCommand_Request * msg)
{
  if (!msg) {
    return false;
  }
  // manual_mode
  // emergency_stop
  // gear
  return true;
}

void
t870_msgs__srv__ModeCommand_Request__fini(t870_msgs__srv__ModeCommand_Request * msg)
{
  if (!msg) {
    return;
  }
  // manual_mode
  // emergency_stop
  // gear
}

bool
t870_msgs__srv__ModeCommand_Request__are_equal(const t870_msgs__srv__ModeCommand_Request * lhs, const t870_msgs__srv__ModeCommand_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // manual_mode
  if (lhs->manual_mode != rhs->manual_mode) {
    return false;
  }
  // emergency_stop
  if (lhs->emergency_stop != rhs->emergency_stop) {
    return false;
  }
  // gear
  if (lhs->gear != rhs->gear) {
    return false;
  }
  return true;
}

bool
t870_msgs__srv__ModeCommand_Request__copy(
  const t870_msgs__srv__ModeCommand_Request * input,
  t870_msgs__srv__ModeCommand_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // manual_mode
  output->manual_mode = input->manual_mode;
  // emergency_stop
  output->emergency_stop = input->emergency_stop;
  // gear
  output->gear = input->gear;
  return true;
}

t870_msgs__srv__ModeCommand_Request *
t870_msgs__srv__ModeCommand_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  t870_msgs__srv__ModeCommand_Request * msg = (t870_msgs__srv__ModeCommand_Request *)allocator.allocate(sizeof(t870_msgs__srv__ModeCommand_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(t870_msgs__srv__ModeCommand_Request));
  bool success = t870_msgs__srv__ModeCommand_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
t870_msgs__srv__ModeCommand_Request__destroy(t870_msgs__srv__ModeCommand_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    t870_msgs__srv__ModeCommand_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
t870_msgs__srv__ModeCommand_Request__Sequence__init(t870_msgs__srv__ModeCommand_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  t870_msgs__srv__ModeCommand_Request * data = NULL;

  if (size) {
    data = (t870_msgs__srv__ModeCommand_Request *)allocator.zero_allocate(size, sizeof(t870_msgs__srv__ModeCommand_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = t870_msgs__srv__ModeCommand_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        t870_msgs__srv__ModeCommand_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
t870_msgs__srv__ModeCommand_Request__Sequence__fini(t870_msgs__srv__ModeCommand_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      t870_msgs__srv__ModeCommand_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

t870_msgs__srv__ModeCommand_Request__Sequence *
t870_msgs__srv__ModeCommand_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  t870_msgs__srv__ModeCommand_Request__Sequence * array = (t870_msgs__srv__ModeCommand_Request__Sequence *)allocator.allocate(sizeof(t870_msgs__srv__ModeCommand_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = t870_msgs__srv__ModeCommand_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
t870_msgs__srv__ModeCommand_Request__Sequence__destroy(t870_msgs__srv__ModeCommand_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    t870_msgs__srv__ModeCommand_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
t870_msgs__srv__ModeCommand_Request__Sequence__are_equal(const t870_msgs__srv__ModeCommand_Request__Sequence * lhs, const t870_msgs__srv__ModeCommand_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!t870_msgs__srv__ModeCommand_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
t870_msgs__srv__ModeCommand_Request__Sequence__copy(
  const t870_msgs__srv__ModeCommand_Request__Sequence * input,
  t870_msgs__srv__ModeCommand_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(t870_msgs__srv__ModeCommand_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    t870_msgs__srv__ModeCommand_Request * data =
      (t870_msgs__srv__ModeCommand_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!t870_msgs__srv__ModeCommand_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          t870_msgs__srv__ModeCommand_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!t870_msgs__srv__ModeCommand_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
t870_msgs__srv__ModeCommand_Response__init(t870_msgs__srv__ModeCommand_Response * msg)
{
  if (!msg) {
    return false;
  }
  // success
  return true;
}

void
t870_msgs__srv__ModeCommand_Response__fini(t870_msgs__srv__ModeCommand_Response * msg)
{
  if (!msg) {
    return;
  }
  // success
}

bool
t870_msgs__srv__ModeCommand_Response__are_equal(const t870_msgs__srv__ModeCommand_Response * lhs, const t870_msgs__srv__ModeCommand_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  return true;
}

bool
t870_msgs__srv__ModeCommand_Response__copy(
  const t870_msgs__srv__ModeCommand_Response * input,
  t870_msgs__srv__ModeCommand_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  return true;
}

t870_msgs__srv__ModeCommand_Response *
t870_msgs__srv__ModeCommand_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  t870_msgs__srv__ModeCommand_Response * msg = (t870_msgs__srv__ModeCommand_Response *)allocator.allocate(sizeof(t870_msgs__srv__ModeCommand_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(t870_msgs__srv__ModeCommand_Response));
  bool success = t870_msgs__srv__ModeCommand_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
t870_msgs__srv__ModeCommand_Response__destroy(t870_msgs__srv__ModeCommand_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    t870_msgs__srv__ModeCommand_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
t870_msgs__srv__ModeCommand_Response__Sequence__init(t870_msgs__srv__ModeCommand_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  t870_msgs__srv__ModeCommand_Response * data = NULL;

  if (size) {
    data = (t870_msgs__srv__ModeCommand_Response *)allocator.zero_allocate(size, sizeof(t870_msgs__srv__ModeCommand_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = t870_msgs__srv__ModeCommand_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        t870_msgs__srv__ModeCommand_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
t870_msgs__srv__ModeCommand_Response__Sequence__fini(t870_msgs__srv__ModeCommand_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      t870_msgs__srv__ModeCommand_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

t870_msgs__srv__ModeCommand_Response__Sequence *
t870_msgs__srv__ModeCommand_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  t870_msgs__srv__ModeCommand_Response__Sequence * array = (t870_msgs__srv__ModeCommand_Response__Sequence *)allocator.allocate(sizeof(t870_msgs__srv__ModeCommand_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = t870_msgs__srv__ModeCommand_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
t870_msgs__srv__ModeCommand_Response__Sequence__destroy(t870_msgs__srv__ModeCommand_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    t870_msgs__srv__ModeCommand_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
t870_msgs__srv__ModeCommand_Response__Sequence__are_equal(const t870_msgs__srv__ModeCommand_Response__Sequence * lhs, const t870_msgs__srv__ModeCommand_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!t870_msgs__srv__ModeCommand_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
t870_msgs__srv__ModeCommand_Response__Sequence__copy(
  const t870_msgs__srv__ModeCommand_Response__Sequence * input,
  t870_msgs__srv__ModeCommand_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(t870_msgs__srv__ModeCommand_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    t870_msgs__srv__ModeCommand_Response * data =
      (t870_msgs__srv__ModeCommand_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!t870_msgs__srv__ModeCommand_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          t870_msgs__srv__ModeCommand_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!t870_msgs__srv__ModeCommand_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
