...
| 토픽 이름 | 메시지 타입 | QoS | 설명 |
|---|---|---|---|
| `/perception/raw_lane_boundaries` | `ev_msgs/msg/LaneBoundaryArray` | BestEffort, depth=1 | 원본 차선 경계 (인스턴스별 lane_id 포함) → lane_chaining 노드로 전달 |
...
yolo_instance_seg_node (/perception/raw_lane_boundaries)
    │
    ▼
lane_chaining_node (/perception/lane_boundaries)
    │
    ▼
