...
class ClusterCounterNode(Node):
    def __init__(self, topic_name: str = '/pointcloud/clustered'):
        ...
        self.get_logger().info(f'cluster_counter started topic={self._topic_name} (print every 1s)')
    ...
    def _print_status(self):
        if self._latest_total is None:
            self.get_logger().info(f'clusters: waiting for {self._topic_name} ...')
            return
...
def main():
    topic_name = '/pointcloud/clustered'
    if len(sys.argv) >= 2:
        topic_name = sys.argv[1]

    rclpy.init()
    node = ClusterCounterNode(topic_name)
    try:
        rclpy.spin(node)
